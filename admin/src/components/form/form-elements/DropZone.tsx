...
const DropzoneComponent: React.FC<DropzoneComponentProps> = ({ onChange, onValidate }) => {
  const [files, setFiles] = useState<File[]>([]);

  const onDrop = (acceptedFiles: File[]) => {
    setFiles(acceptedFiles);
    onChange(acceptedFiles); // Send files to parent
  };
  ...
  useEffect(() => {
    if (onValidate) {
      onValidate(() => files.length > 0); // Validation: At least one file uploaded
    }
  }, [files, onValidate]);

  return (
    <ComponentCard title="Kitob rasmini yuklash">
        ...
        >
          ...
          <div className="dz-message flex flex-col items-center">
            ...
            <h4 className="mb-3 font-semibold text-center text-gray-800 text-theme-xl dark:text-white/90">
              {isDragActive ? "Rasmni qo'shish" : "Rasmlarni yuklash uchun bosing"}
            </h4>
            <span className="text-center mb-5 block w-full max-w-[290px] text-sm text-gray-700 dark:text-gray-400">
              Iltimos PNG, JPG, WebP, SVG fayllarni yuklang.
            </span>
