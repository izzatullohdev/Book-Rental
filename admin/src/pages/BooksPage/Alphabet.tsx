...
const Alphabet = () => {
  ...
  const fetchGroups = async () => {
    ...
    try {
      ...
      setError("Alifboni olishda xatolik yuz berdi.")
    } finally {
      setFetchLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (!name.trim()) {
      antdMessage.warning("Alifbo kiritish shart!")
      return
    }
    ...
    try {
      ...
      antdMessage.success("Alifbo muvaffaqiyatli qo'shildi!")
      setName("")
      await fetchGroups()
    } catch (error) {
      console.error("Xatolik yuz berdi:", error)
      antdMessage.error("Alifbo qo'shilmadi!")
    } finally {
  ...
  const handleUpdateOk = async () => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_API}/api/alphabet/${selectedGroup?.id}`,
        { name: editedTitle },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      fetchGroups()
      antdMessage.success("Alifbo muvaffaqiyatli yangilandi!")
    } catch (error) {
  ...
  const handleDeleteOk = async () => {
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_API}/api/alphabet/${selectedGroup?.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "X-permission": permissionIds[0],
        },
  ...
  return (
    <div className="min-h-[80%] p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-auto">
      <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white">Kitob alifbosini qo'shish</h2>
      <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-6">
        <div className="w-full md:col-span-2">
          <label htmlFor="alphabet" className="block font-medium text-gray-700 dark:text-gray-300 mb-1">
            Kitob alifbosi
          </label>
          <input
            id="alphabet"
            ...
            placeholder="Masalan: O'zbek yoki kirilcha"
      ...
      <div className="mt-20">
        <h2 className="text-2xl font-medium mb-6 text-gray-800 dark:text-white">
          {groups.length === 0 ? "Alifbolar yo'q!" : "Barcha alifbolar"}
        </h2>
        {fetchLoading ? (
      ...
      >
        <Input value={editedTitle} onChange={(e) => setEditedTitle(e.target.value)} placeholder="Yangi Alifbo nomi" />
      </Modal>
      {/* DELETE MODAL */}
      <Modal
        title="Alifboni o'chirish"
        ...
        cancelText="Yo'q"
      >
        <p>{selectedGroup ? `"${selectedGroup.name}" alifboni o'chirmoqchimisiz?` : ""}</p>
      </Modal>
    </div>
  )
}
