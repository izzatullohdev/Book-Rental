...
const Auther = () => {
  ...
  const handleUpdateOk = async () => {
    ...
    try {
      ...
      fetchGroups()
      antdMessage.success("Muallif muvaffaqiyatli yangilandi!")
    } catch (error) {
  ...
  const handleDeleteOk = async () => {
    try {
      ...
      const matchedGroups = userGroup.filter((item) => isRoles.includes(item.group_id))
      const permissionIds = matchedGroups?.map((item) => item.permissionInfo.code_name)
      await axios.delete(`${import.meta.env.VITE_API}/api/auther/${selectedGroup?.id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
          "X-permission": permissionIds[0],
        },
    ...
    <div className="min-h-[80%] p-6 bg-white dark:bg-gray-800 rounded-xl shadow-md overflow-auto">
      ...
      <div className="mt-20">
        <h2 className="text-2xl font-medium mb-6 text-gray-800 dark:text-white">
          {groups.length === 0 ? "Kitob mualliflari yo'q!" : "Barcha kitob mualliflari"}
        </h2>
        {fetchLoading ? (
      ...
      >
        <Input value={editedTitle} onChange={(e) => setEditedTitle(e.target.value)} placeholder="Yangi muallif nomi" />
      </Modal>
